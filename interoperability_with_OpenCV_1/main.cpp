...
static void help(char *progName)
{
    cout << endl
         << progName
         << "show how to use cv::Mat and IplImages together (converting back and forth)." << endl
         ...
         << "Usage:" << endl
         << progName << "[image-name Default: ../data/lena.jpg]" << endl
         << endl;
}
...
{
    help(argv[0]);
    const char *imagename = argc > 1 ? argv[1] : "addingImages/lena.jpg";

#ifdef DEMO_MIXED_API_USE

    //Ptr<T> is a safe ref-counting pointer class
    ...
    //Convert to the new style container.Only header created.Image not copied.
    Mat I = cv::cvarrToMat(IplI);
    cvShowImage(imagename, IplI);
#else
    ...
    //Fill the matrix with normally distributed random values(around number with deviation off)
    ...
    //![end]
    merge(planes, I_YUV);
    cvtColor(I_YUV, I, COLOR_YCrCb2BGR);
